...
static char
project_doc[] = ".. py:method:: Camera.project(vector, aspect) \n"
"\n"
"Apply the projection of the camera to a vector with a given aspect ratio\n"
"\n"
":param soy.atoms.Vector vector: vector to be projected \n"
":param float aspect: aspect ratio \n"
"\n";

static PyObject*
project (SELF self, PyObject* args) {
    PyObject* vector;
    float aspect;

    // Parse arguments
    if (!PyArg_ParseTuple(args, "O!f", &PySoy_atoms_Vector_Type, &vector, &aspect))
        return NULL;

    soyatomsVector* v = soy_bodies_camera_projectVector(self->g, ((PySoy_atoms_Vector_Object*)vector)->g, aspect);
    
    PyObject* result = Py_BuildValue("(fff)", soy_atoms_vector_get_x(v), soy_atoms_vector_get_y(v), soy_atoms_vector_get_z(v));
...
static char
lens_doc[] = "..py:attribte:: Lens(angle=45.0)\n"
"    Camera lens angle.\n"
"\n"
":param angle: This is the (vertical) angle which the lens can view.\n"
":type angle: float\n"
"\n";
