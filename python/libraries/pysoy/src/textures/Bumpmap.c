...
tp_new (PyTypeObject* type, PyObject* args, PyObject* kwds) {
    ...
    if (!PyArg_ParseTuple(args, "s", &filename)) {
        ...
        if (!PyArg_ParseTuple(args, "")) {
            ...
            if (!PyArg_ParseTuple(args, "O!|f",
                          &PySoy_textures_Texture_Type, &texture, &strength)) {
                return NULL;
            }
        }
    }
    ...
    if (filename) {
        ...
        jpg = g_str_has_suffix(filename, ".jpg") ||
    ...
    // new gobject for JPG file
    else if (jpg)
        self->g = soy_textures_bumpmap_new_from_jpg(filename, &error);
    // new gobject for texture
    else if (texture)
        self->g = soy_textures_bumpmap_new_emboss(texture->g, strength);
    // new gobject
    else
