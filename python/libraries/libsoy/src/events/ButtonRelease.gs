    ...
    def executeBinding()
        if(_bindings != null)
            for b in _bindings.keys 
                if b == _button
                    _bindings[b].hid_touch_up(_button, (int) _pointer.position.x, (int) _pointer.position.y)
	...
	def decodeKey(value : uint)
        ...
        case value
            ...
            default
                ...
                if( value != 4) and ( value != 5)                    //Don't warn WheelUp and WheelDn Release
                    print "Unknown button %u released", value
