...
class soy.textures.Cubemap : soy.textures.Texture
    ...
    def _update_texture (texture : soy.textures.Texture?, target : GLenum)
        if texture is not null and texture.updated
            texture.update(target)
            texture.updated = false

    def _connect_texture (texture : soy.textures.Texture?, target : GLenum)
    ...
    def override enable ()
        ...
        if _texture is 0
            ...
            // flag the texture for updating here so itll be processed below
            self._updated = true
        ...
        if self._updated
            ...
            self._connect_texture(_right, GL_TEXTURE_CUBE_MAP_POSITIVE_X)
            self._connect_texture(_left, GL_TEXTURE_CUBE_MAP_NEGATIVE_X)
            self._connect_texture(_up, GL_TEXTURE_CUBE_MAP_POSITIVE_Y)
            self._connect_texture(_down, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y)
            self._connect_texture(_front, GL_TEXTURE_CUBE_MAP_POSITIVE_Z)
            self._connect_texture(_back, GL_TEXTURE_CUBE_MAP_NEGATIVE_Z)

            _updated = false

        self._update_texture(_right, GL_TEXTURE_CUBE_MAP_POSITIVE_X)
        self._update_texture(_left, GL_TEXTURE_CUBE_MAP_NEGATIVE_X)
        self._update_texture(_up, GL_TEXTURE_CUBE_MAP_POSITIVE_Y)
        self._update_texture(_down, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y)
        self._update_texture(_front, GL_TEXTURE_CUBE_MAP_POSITIVE_Z)
        self._update_texture(_back, GL_TEXTURE_CUBE_MAP_NEGATIVE_Z)
